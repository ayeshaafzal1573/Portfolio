...
export function HireMeSection() {
  ...
  const mailto = `mailto:${contactData?.email || "ayeshaafzal1573@gmail.com"}?subject=Collaboration%20Opportunity`
  const resumeUrl = contactData?.resume_url || "https://drive.google.com/file/d/1GGuBWHrTkwG982hPpZNWLrSMPMc7qOoA/view?usp=sharing"
        ...
        <div className="glass-card rounded-[2.5rem] p-8 md:p-16 text-center relative overflow-hidden group shadow-2xl reveal-scale border border-[color:var(--card-border)] bg-[color-mix(in_srgb,var(--surface)_80%,transparent)]">
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-4 pt-4 reveal delay-300">
              <a href={mailto} className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-8 py-4 btn-primary text-white font-extrabold rounded-full shadow-lg shadow-[var(--ring-soft)] hover:scale-[1.05] hover:shadow-xl transition-all duration-300 group/btn">
                <Mail className="w-5 h-5 transition-transform duration-300 group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5" />
                <span>Email Me</span>
              </a>
              <a href={resumeUrl} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-7 py-3.5 btn-secondary font-bold rounded-full shadow-sm hover:scale-[1.05] hover:text-[color:var(--accent-primary)] hover:border-[color:var(--accent-primary)] hover:bg-[color:var(--accent-soft)] transition-all duration-300 group/btn">
              ...
              <a href="https://github.com/ayeshaafzal1573" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-7 py-3.5 btn-secondary font-bold rounded-full shadow-sm hover:scale-[1.05] hover:text-[color:var(--accent-primary)] hover:border-[color:var(--accent-primary)] hover:bg-[color:var(--accent-soft)] transition-all duration-300 group/btn">
                <Github className="w-5 h-5 text-[color:var(--accent-primary)] transition-transform duration-300 group-hover/btn:scale-110" />
                <span>GitHub</span>
              </a>
              <a href="https://www.linkedin.com/in/ayeshaafzalqadir/" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-7 py-3.5 btn-secondary font-bold rounded-full shadow-sm hover:scale-[1.05] hover:text-[color:var(--accent-primary)] hover:border-[color:var(--accent-primary)] hover:bg-[color:var(--accent-soft)] transition-all duration-300 group/btn">
